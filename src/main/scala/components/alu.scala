...
class ALU extends Module {
  ...
  when (io.operation === "b00001".U) { // add
    io.result := io.operand1 + io.operand2
  }
  .elsewhen (io.operation === "b00000".U) { // addw
    io.result := signExtend32To64(operand1_32 + operand2_32) // + results in width of max(width(op1), width(op2))
  }
  .elsewhen (io.operation === "b00100".U) { // sub
    io.result := io.operand1 - io.operand2
  }
  .elsewhen (io.operation === "b00010".U) { // subw
    io.result := signExtend32To64(operand1_32 - operand2_32)
  }
  .elsewhen (io.operation === "b00110".U) { // mul
    io.result := io.operand1 * io.operand2
  }
  .elsewhen (io.operation === "b00101".U) { // mulw
  ...
  .elsewhen (io.operation === "b01011".U) { // div
  ...
  .elsewhen (io.operation === "b01001".U) { // divw
